...
Seed는 장기 활성 샘플 이벤트 1개만 upsert합니다. 매장 데이터는 `npm run stores:sync`로 메가MGC커피 공식 매장찾기에서 동기화합니다. 동기화는 공식 응답이 4,000개 미만이면 중단하며 신규 매장은 추가하고 변경 매장은 갱신하고 더 이상 조회되지 않는 공식 매장은 비활성화합니다. 비밀번호나 가짜 카카오 사용자는 seed하지 않습니다.
...
전체 목록은 `.env.example`에 있습니다. `JWT_ACCESS_SECRET`은 최소 64자, `COOKIE_SECRET`은 최소 32자여야 합니다. `CORS_ORIGINS`는 명시적 allowlist이며 `*`는 거부됩니다. 운영은 `COOKIE_SECURE=true`가 필수이고, cross-site cookie에서는 `COOKIE_SAME_SITE=none`과 Secure를 함께 사용해야 합니다. `ADMIN_KAKAO_USER_IDS`에 있는 카카오 회원번호의 사용자는 로그인 시 ADMIN이 됩니다. 프로필 이미지 파일은 `UPLOAD_DIR`에 저장되므로 운영에서는 이 경로를 영속 볼륨과 백업 대상에 포함하고, `PUBLIC_BASE_URL`에는 외부 HTTPS origin을 지정합니다.
