...
type Post = {
  ...
  createdAt: string;
  updatedAt: string;
  writer: { id: string; nickname: string; profileImage: string | null; rating: number; reviewCount: number };
  store: Store;
...
닉네임은 trim 후 2~20자이며 중복되면 `NICKNAME_ALREADY_EXISTS`(409)입니다. 프론트에서 프로필 이미지 URL을 직접 수정하는 UI는 만들지 않습니다.

### 매장

| Method | Path | 인증 | 설명 |
| --- | --- | --- | --- |
| GET | `/stores?region=&keyword=&page=1&limit=20` | 없음 | 매장 목록/검색 |
| GET | `/stores/:id` | 없음 | 매장 상세 |

`region`은 정확한 지역명 비교, `keyword`는 매장명/주소 부분 검색입니다. 매장 선택 UI는 ID를 모집글 `storeId`로 사용합니다.
...
후기는 마감(`CLOSED`) 글에만 작성할 수 있고, 작성자 본인에게는 작성할 수 없으며 글당 1회입니다. 서버는 판매자를 글 작성자로 결정하므로 클라이언트가 `sellerId`를 보내지 않습니다. 후기 제출 성공 후 판매자의 `rating`, `reviewCount`가 서버 transaction 안에서 갱신됩니다.

> 현재 MVP에는 실제 참여 신청 모델이 없습니다. 즉 “참여자만 후기 작성”은 아직 검증하지 못하며, 프론트도 이 기능을 참여 인증처럼 표현하면 안 됩니다.
...
| 신고 처리 | `PATCH /admin/reports/:id` | `{ status: "RESOLVED" | "REJECTED", adminNote }` |
...
| 내 프로필 | `GET /auth/me` | 닉네임은 `PATCH /users/me` |
| 다른 사용자 프로필 | `GET /users/:id` | 작성글/후기 목록 조회 |
| 후기 작성 | 상세 글 상태 확인 | `POST /reviews` |
