...
`comments`는 `votes`와 동일하게 `campaign_id`, `session_id`, `page_view_id`, `request_id`, `visitor_hash`, `network_hash`를 가지며 `(visitor_hash, request_id)` unique입니다. `submit_comment`는 캠페인이 `active`일 때만, 그리고 같은 `(campaign, visitor, choice)`로 이미 유효한 vote가 있을 때만 댓글을 허용합니다. rate limit은 `claim_comment_attempt`(방문자 10분당 5건·네트워크 10분당 50건)를 그대로 재사용합니다. `list_public_comments`는 최근 댓글만 신원 정보 없이 공개합니다.

## 이전 주제

`campaign_settings`는 `singleton boolean unique check (singleton)` 제약으로 항상 정확히 한 행만 존재하므로, DB에는 "여러 주제"라는 개념이 없습니다. 주제를 바꿀 때는 `archive_current_topic_and_reset(title, option_a_label, option_a_choice, option_b_label, option_b_choice, reason)`을 service-role 권한으로 한 번 호출합니다. 이 RPC는 `vote_count_shards` 합계를 `topic_history`에 스냅샷으로 남기고 같은 transaction에서 64개 shard를 0으로 리셋합니다. 원본 `votes` 행은 삭제하지 않으므로 과거 집계도 감사 가능합니다. `list_public_topic_history`가 공개 목록을 제공합니다.
...
| `submit_comment` | 캠페인·session·vote 보유·rate limit 확인 후 댓글 insert |
| `list_public_comments` | 최근 댓글 공개 목록 |
| `archive_current_topic_and_reset` | 현재 집계를 `topic_history`에 스냅샷하고 shard 리셋 |
| `list_public_topic_history` | 이전 주제 공개 목록 |
| `cleanup_operational_data` | 48시간보다 오래된 vote/analytics rate bucket 삭제 |

`active`는 전체 기능, `protected`는 투표와 공개 결과만, `read_only` 또는 window 밖은 공개 결과만 허용합니다. 댓글은 `protected`에서도 막히며 `active`에서만 허용됩니다. 변경 RPC는 DB 설정을 같은 transaction에서 읽으므로 앱 cache나 재시작 없이 다음 mutation부터 적용됩니다.
...
`cleanup_operational_data()`는 `vote_rate_buckets`와 `analytics_rate_buckets`만 정리합니다. VM의 hourly timer 또는 하나의 `pg_cron` schedule 중 하나로 실행합니다. 상세 분석 원본은 자동 만료하지 않으며 운영 후 CSV 백업과 Supabase 수동 삭제 절차를 따릅니다. `comment_attempts`도 `claim_comment_attempt` 내부에서 30일이 지난 행을 스스로 정리합니다.
