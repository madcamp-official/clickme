...
import { CRITICAL_DATABASE_RESERVE, tryAcquireDatabase, voteCapacity } from "../../../lib/server/capacity";
...
export async function POST(request: NextRequest) {
  ...
  if (!isChoice(choice) || !isUuid(requestId) || !isUuid(sessionId) || !isUuid(pageViewId)) {
  ...
  const release = tryAcquireDatabase(voteCapacity, CRITICAL_DATABASE_RESERVE);
  if (!release) {
