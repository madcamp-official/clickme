...
export async function POST(request: NextRequest) {
  ...
  if (!isUuid(idempotencyKey) || !isUuid(sessionId) || !isUuid(pageViewId) || !isTeamChoice(choice)) {
  ...
  const releaseCreate = teamShareCreateCapacity.tryAcquire();
  ...
  try {
    ...
    if (error?.message.includes("sharing_disabled") || error?.message.includes("campaign_not_active")) return apiError(410, "현재 추천 링크를 만들 수 없습니다.", "SHARING_DISABLED");
    if (error || !data) return apiError(503, "공유 링크를 만들 수 없습니다.", "SERVICE_UNAVAILABLE");

    let imagePath = data.image_path;
    if (data.created) {
      try {
        const png = await renderTeamShareCard(choice, data.vote_count, data.total_count);
        imagePath = await storeShareCard(data.campaign_id, data.share_id, png, "team_share_links");
      } catch {
