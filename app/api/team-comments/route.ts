...
import { formatTeamComments } from "../../../lib/server/team-comments";
...
export async function GET() {
  const release = tryAcquireDatabase(teamCommentReadCapacity, CRITICAL_DATABASE_RESERVE);
  if (!release) {
  ...
  try {
    const { data, error } = await getSupabaseAdmin()
      .rpc("list_public_team_comments", { p_limit: 50 })
    ...
    return jsonPublicResult(formatTeamComments(data));
  } catch {
    return apiError(503, "댓글을 불러올 수 없습니다. 잠시 후 다시 시도해 주세요.", "SERVICE_UNAVAILABLE");
  } finally {
    release();
...
export async function POST(request: NextRequest) {
  ...
  const release = tryAcquireDatabase(teamCommentCapacity, CRITICAL_DATABASE_RESERVE);
  if (!release) {
