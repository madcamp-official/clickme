...
import { boundedInteger } from "../../../../lib/server/analytics-validation";
...
export async function POST(request: NextRequest) {
  ...
  if (!body.ok) return apiError(body.reason === "too_large" ? 413 : 400, "heartbeat 형식이 올바르지 않습니다.", body.reason === "too_large" ? "PAYLOAD_TOO_LARGE" : "INVALID_HEARTBEAT");

  const { sessionId, pageViewId } = body.value;
  const sequence = boundedInteger(body.value.sequence, 1, 1_000_000);
  const visibleMs = boundedInteger(body.value.visibleMs, 0, 86_400_000);
  const activeMs = boundedInteger(body.value.activeMs, 0, 86_400_000);
  const maxScrollPercent = boundedInteger(body.value.maxScrollPercent, 0, 100);
  if (!isUuid(sessionId) || !isUuid(pageViewId) || sequence === null || visibleMs === null || activeMs === null || activeMs > visibleMs || maxScrollPercent === null) {
    return apiError(400, "heartbeat 형식이 올바르지 않습니다.", "INVALID_HEARTBEAT");
  }
